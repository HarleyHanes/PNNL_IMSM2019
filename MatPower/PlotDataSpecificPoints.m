...
title('p versus q values for all buses');
hold on
vline(pmn,'k','maximum p min for all gen')
hline(qmx,'r','minimum q max for all gen')
hline(qmn,'r','maximum q min for all gen')
vline(pmx,'k','minimum p max for all gen')
scatterS=scatter(cell2mat(XCol3S(1:25:end)), cell2mat(YCol4S(1:25:end)),'b');
scatterS.MarkerEdgeAlpha=0.2;
legend('fail values','suc values')
xlabel('p values');
ylabel('q values');
...
hline(CSmax,'b');
hline(CSmin,'b');
title('Cost of failures(red) and successes(blue)');
text(22000,775,['Fail Max $',num2str(round(CFmax,2))],'Color','r')
text(22000,579,['Fail Min $',num2str(round(CFmin,2))],'Color','r')
text(22000,681,['Suc Max $',num2str(round(CSmax,2))],'Color','b')
text(22000,465,['Suc Min $',num2str(round(CSmin,2))],'Color','b')
xlabel('Simulation Number')
ylabel('Cost')
